...
>>>>>>> a809cb3 (Fix evolve-step workspace cleanup)
    ...
    async def test_handle_uses_parent_cwd_when_dirty_delegated_workspace_skipped(self) -> None:
        """Dirty delegated parents should bypass child worktree creation."""
        mock_event_store = AsyncMock()
        mock_event_store.initialize = AsyncMock()
        mock_runner = MagicMock()
        mock_runner.execute_seed = AsyncMock(
        ...
        with (
            ...
            patch(
                "ouroboros.mcp.tools.definitions.maybe_prepare_task_workspace",
                return_value=None,
            ) as prepare_mock,
            patch(
                "ouroboros.mcp.tools.definitions.OrchestratorRunner",
            ...
            result = await handler.handle(
                {
                    "seed_content": VALID_SEED_YAML,
                    DELEGATED_PARENT_SESSION_ID_ARG: "sess_parent",
                    DELEGATED_PARENT_CWD_ARG: "/tmp/project",
                    DELEGATED_PARENT_PERMISSION_MODE_ARG: "bypassPermissions",
                    "skip_qa": True,
                },
                execution_id="exec_child",
        ...
        assert result.is_ok
        prepare_mock.assert_called_once_with("/tmp/project", "orch_child", allow_dirty=True)
        runner_kwargs = runner_cls.call_args.kwargs
        inherited_handle = runner_kwargs["inherited_runtime_handle"]
        assert inherited_handle is not None
        assert inherited_handle.cwd == "/tmp/project"
        assert runner_kwargs["task_cwd"] is None
        assert runner_kwargs["task_workspace"] is None

    async def test_handle_runs_without_workspace_when_source_cwd_is_not_git_repo(self) -> None:
        """Non-git executions should proceed without a managed task workspace."""
        mock_event_store = AsyncMock()
        mock_event_store.initialize = AsyncMock()
        mock_runner = MagicMock()
        mock_runner.execute_seed = AsyncMock(
            return_value=Result.ok(
                OrchestratorResult(
                    success=True,
                    session_id="orch_nongit",
                    execution_id="exec_nongit",
                    final_message="Done",
                )
            )
        )
        ...
        with (
            ...
            result = await handler.handle(
                ...
                execution_id="exec_nongit",
                session_id_override="orch_nongit",
            )
